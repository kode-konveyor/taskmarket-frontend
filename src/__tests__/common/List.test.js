...
const persons = [{ _id: 1, name: 'John Doe' }, { _id: 2, name: 'Karcsi Kadlot' }]
...
beforeEach(() => {
  renderedComponent = shallow(<List Of={Person} data={persons} className='personList'/>, { disableLifecycleMethods: false })
})
...
it('renders empty list when data is not an array', () => {
  renderedComponent = shallow(<List Of={Person} className='personList'/>, { disableLifecycleMethods: false })
